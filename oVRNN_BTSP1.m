function [os,xs,Vs,ds,wbcos,P] = oVRNN_BTSP1(modeltype,pp,abase,g,decay_rate,ndim,nt,P,plotyn)

% oVRNN_BTSP1, modified from rnrl101coslra
%   modeltype: 3:RevVRNNbp, 4:BioVRNNrf, 5:untrained RNN, 6:BioVRNNrf with (pp*pre-post + (1-pp)*post-pre) potentiation
%   pp: proportion of pre-post potentiation (1:pre-post only, 0:post-pre only), valid only for modeltype=6
%   abase: learning rate base, e.g., 0.1
%   g: time discount factor, e.g., 0.8
%   decay_rate: rate of value-weight-decay per time-step, e.g., 0.001
...
% C     R     e e e  e
%
%   [os,xs,Vs,ds,wbcos,P] = oVRNN_BTSP1(6,2/3,0.1,0.8,0.0005,40,1500,[],1);
%   [os,xs,Vs,ds,wbcos,P] = oVRNN_BTSP1(6,3/5,0.1,0.8,0.0005,40,1500,[],1);
...
for t = 2:tmax
    ...
    if t >= 3
        ...
        elseif modeltype == 6
            y1 = (xs(:,t-1)<0.5).*xs(:,t-1).*(1-xs(:,t-1)) + (xs(:,t-1)>=0.5)*0.5*0.5;
            y2 = (xs(:,t-2)<0.5).*xs(:,t-2).*(1-xs(:,t-2)) + (xs(:,t-2)>=0.5)*0.5*0.5;
            P = P + abase*ds(t-1)*(pp*(y1.*b')*[xs(:,t-2);os(:,t-2)]' + (1-pp)*(y2.*b')*[xs(:,t-1);os(:,t-1)]');
        end
        w = wnew;
    end
    w = w * (1 - decay_rate);
