function [os,xs,Vs,ds,wbcos,P] = rnrl101cos(modeltype,a,g,ndim,nt,P,plotyn)
...
% C     R     e e e  e
%
%   [os,xs,Vs,ds,wbcos,P] = rnrl101cos(4,0.1,0.8,12,1500,[],1);
...
for t = 2:tmax
    ...
    if t >= 3
        ...
        if ((modeltype == 1) && (xs(1,t) == 1))
            ...
            wnew = max(0, w + a*ds(t-1)*xs(:,t-1)');
        end
        if modeltype == 3
            P = P + a*ds(t-1)*(xs(:,t-1).*(1-xs(:,t-1)).*w')*[xs(:,t-2);os(:,t-2)]';
        elseif modeltype == 4
            %P = P + a*ds(t-1)*(xs(:,t-1).*(1-xs(:,t-1)).*b')*[xs(:,t-2);os(:,t-2)]';
            y = (xs(:,t-1)<0.5).*xs(:,t-1).*(1-xs(:,t-1)) + (xs(:,t-1)>=0.5)*0.5*0.5;
            P = P + a*ds(t-1)*(y.*b')*[xs(:,t-2);os(:,t-2)]';
        end
        w = wnew;
    end
end
