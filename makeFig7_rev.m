%
clear all
simseed = setrandoms(25000);

%
ndim = 20;
nsim = 100;
P_set = NaN(ndim,ndim+2,2,nsim);
for modeltype = 3:5
    for tasktype = 1:2
        for k = 1:2
        ...
        for ksim = 1:nsim
            ...
            [os,xs,Vs,ds,trialtypes,P] = rnrl102lra(modeltype,tasktype,0.1,0.8,ndim,2000,[],0);
            if modeltype == 4
                P_set(:,:,tasktype,ksim) = P;
            end
            for k = 1:2
...
for modeltype = 5
    for tasktype = 1:2
        ...
        for ksim = 1:nsim
            ...
            tmp_P = reshape(P_set(:,:,tasktype,ksim),ndim*(ndim+2),1);
            tmp_P = tmp_P(randperm(ndim*(ndim+2)));
            P = reshape(tmp_P,ndim,ndim+2);
            [os,xs,Vs,ds,trialtypes,P] = rnrl102lra(modeltype,tasktype,0.1,0.8,ndim,2000,P,0);
            for k = 1:2
                tmp_last_cue = find(trialtypes==k,1,'last');
...
save(['data_lrmakeFig8_' num2str(simseed)],'data');
...
for modeltype = 3:6
    for tasktype = 1:2
        ...
        print(F,'-depsc',['Fig7' tmp_ABCD(modeltype-2) num2str(tasktype)]);
    end
end
