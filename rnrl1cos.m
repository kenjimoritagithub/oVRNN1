function [os,xs,Vs,ds,wbcos] = rnrl1cos(modeltype,a,g,ndim,nt,plotyn)
...
% C     R     e e e  e
%
%   [os,xs,Vs,ds,wbcos] = rnrl1cos(4,0.1,0.8,7,1000,1);
...
for t = 2:tmax
    ...
    if t >= 3
        ...
        if ((modeltype == 1) && (xs(1,t) == 1))
            ...
            wnew = w + a*ds(t-1)*xs(:,t-1)';
        end
        if modeltype == 3
            P = P + a*ds(t-1)*((0.5+xs(:,t-1)).*(0.5-xs(:,t-1)).*w')*[xs(:,t-2);os(:,t-2)]';
        elseif modeltype == 4
            P = P + a*ds(t-1)*((0.5+xs(:,t-1)).*(0.5-xs(:,t-1)).*b')*[xs(:,t-2);os(:,t-2)]';
        end
        w = wnew;
    end
end
